...
def preparar_para_mostrar(df, columnas_texto=None, columnas_entero=None, columnas_decimal=None):
    ...
    para evitar fallos de conversión PyArrow por tipos mixtos.
    """
...
def resumen_por_cartera(df):
    ...
    total de CAPITAL, # CLIENTES, RECAUDO y PROYECCION.
    """
    columnas_salida = [
...
def insertar_registros_supabase(df):
    """Inserta nuevos registros a la base de datos de Supabase."""
    registros = []
    for _, r in df.iterrows():
        cap = float(r["CAPITAL"]) if pd.notna(r["CAPITAL"]) else 0.0
        rec = float(r["RECAUDO"]) if pd.notna(r["RECAUDO"]) else 0.0
        proy = float(r["PROYECCION"]) if pd.notna(r["PROYECCION"]) else 0.0
        efect = (rec / cap * 100) if cap > 0 else 0.0
        est = rec + proy

        registros.append({
            "cartera": str(r["CARTERA"]).strip(),
            "director": str(r["DIRECTOR"]).strip().upper(),
            "mes": str(r["MES"]).strip().upper(),
            "capital": cap,
            "num_clientes": int(r["# CLIENTES"]) if pd.notna(r["# CLIENTES"]) else 0,
            "recaudo": rec,
            "proyeccion": proy,
            "efectividad": efect,
            "estimado_cierre": est,
...
# ============================================================
    ...
    with t1:
        ...
        with c_left:
            st.subheader("🏆 Recaudo Acumulado por Director")
            ...
            fig_dir_g = px.bar(
                ...
                color="DIRECTOR",
                color_discrete_sequence=PALETA_VIVA,
                title="<b>Recaudo Total por Director</b>",
            )
            fig_dir_g.update_traces(
                texttemplate="$ %{y:,.0f}",
                textposition="outside",
            )
            fig_dir_g.update_layout(
                showlegend=False,
                yaxis_title="Recaudo ($)",
                xaxis_title="Director",
            )
            st.plotly_chart(fig_dir_g, use_container_width=True)

        with c_right:
            st.subheader("📊 Recaudo Acumulado por Cartera")
            ...
            fig_cart_g.update_traces(
                texttemplate="$ %{y:,.0f}",
                textposition="outside",
            )
            fig_cart_g.update_layout(
                showlegend=False,
                yaxis_title="Recaudo ($)",
                xaxis_title="Cartera",
            )
            st.plotly_chart(fig_cart_g, use_container_width=True)

        st.markdown("---")

        mostrar_resumen_cartera(
            df_all,
            titulo="📦 Consolidado General por Cartera",
        )

    with t2:
        st.subheader("📅 Consolidado Mes x Mes")

        df_periodo = preparar_periodo(df_all)
        anios = sorted([int(x) for x in df_periodo["AÑO"].dropna().unique()])
        opciones_anio = ["Todos"] + anios
        anio_sel_g = st.selectbox(
            "Seleccione el año:",
            opciones_anio,
            key="ger_anio_mes",
        )

        df_filtrado_g = df_periodo.copy()
        if anio_sel_g != "Todos":
            df_filtrado_g = df_filtrado_g[df_filtrado_g["AÑO"] == int(anio_sel_g)]

        df_mes_tot_g = (
            df_filtrado_g.groupby(["MES_ORDEN", "MES_NOMBRE"], as_index=False)
            .agg({"CAPITAL": "sum", "RECAUDO": "sum", "PROYECCION": "sum"})
            .sort_values("MES_ORDEN")
        )
        ...
        fig_mes_g = px.bar(
            ...
            barmode="group",
            category_orders={"MES_NOMBRE": list(MESES_CALENDARIO.keys())},
            title="<b>Evolución General de Recaudo vs Proyección</b>",
            color_discrete_map={
                "RECAUDO": "#2563eb",
                "PROYECCION": "#f59e0b",
            },
    ...
    with t3:
        st.subheader("📋 Base General Detallada")

        f_col1, f_col2 = st.columns(2)

        directores_list = ["TODOS"] + sorted(df_all["DIRECTOR"].dropna().unique().tolist())
        carteras_list = ["TODAS"] + sorted(df_all["CARTERA"].dropna().unique().tolist())

        with f_col1:
            dir_filtro = st.selectbox("Filtrar por Director:", directores_list)
        with f_col2:
            cart_filtro = st.selectbox("Filtrar por Cartera:", carteras_list)

        df_base_filtrada = df_all.copy()

        if dir_filtro != "TODOS":
            df_base_filtrada = df_base_filtrada[df_base_filtrada["DIRECTOR"] == dir_filtro]

        if cart_filtro != "TODAS":
            df_base_filtrada = df_base_filtrada[df_base_filtrada["CARTERA"] == cart_filtro]

        df_base_mostrar = preparar_para_mostrar(df_base_filtrada)
        ...
        # Descarga de datos
        csv_data = df_base_filtrada.to_csv(index=False).encode("utf-8")
        st.download_button(
            label="📥 Descargar Reporte en CSV",
            data=csv_data,
            file_name="reporte_recaudo_citi_summa.csv",
            mime="text/csv",
        )

    with t4:
        st.subheader("⚙️ Panel de Administración y Carga de Datos")

        st.markdown("#### 📤 Carga Masiva de Archivo (Excel / CSV)")
        st.caption(
            "Asegúrese de que el archivo contenga las columnas: "
            "`CARTERA`, `DIRECTOR`, `MES`, `CAPITAL`, `# CLIENTES`"
        )
        ...
        if archivo_subido is not None:
            try:
                ...
                cols_requeridas = ["CARTERA", "DIRECTOR", "MES", "CAPITAL", "# CLIENTES"]
                cols_faltantes = [c for c in cols_requeridas if c not in df_nuevo.columns]

                if cols_faltantes:
                    st.error(
                        f"❌ El archivo no contiene las columnas requeridas: {', '.join(cols_faltantes)}"
                    )
                else:
                    if "RECAUDO" not in df_nuevo.columns:
                        df_nuevo["RECAUDO"] = 0.0
                    if "PROYECCION" not in df_nuevo.columns:
                        df_nuevo["PROYECCION"] = 0.0

                    st.write("Preview de datos a cargar:")
                    st.dataframe(df_nuevo.head(), use_container_width=True)

                    if st.button("🚀 Reemplazar Base Completa en Supabase", type="primary"):
                        eliminar_todos_registros_supabase()
                        insertar_registros_supabase(df_nuevo)
                        actualizar_sesion_desde_supabase()
                        st.success("✅ Base de datos reemplazada exitosamente en Supabase.")
                        st.rerun()
        ...
        st.markdown("#### 🔄 Restablecer Base de Datos por Defecto")
        if st.button("⚠️ Reiniciar Base de Datos a la Estructura Inicial", type="secondary"):
            try:
                eliminar_todos_registros_supabase()
                df_init = inicializar_base_datos()
                insertar_registros_supabase(df_init)
                actualizar_sesion_desde_supabase()
                st.success("✅ La base de datos ha sido reiniciada exitosamente con los valores iniciales.")
                st.rerun()
            except Exception as e:
                st.error(f"❌ Error al reiniciar la base: {e}")
